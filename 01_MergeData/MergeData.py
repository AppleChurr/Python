...
if len(sys.argv) == 1: # 확장자 명, 파일 경로 입력 안했을 시
    print("py " + sys.argv[0] + " [Data File Extention] [Data File Path 0] [Data File Path 1] ... ")
elif len(sys.argv) == 2: # 파일 경로 입력 안했을 시
    print("py " + sys.argv[0] + " " + sys.argv[1] + " [Data File Path 0] [Data File Path 1] ... ")
    ...
    if(DataExten[0] != '.'):
        ...
        for Argidx in range(2, len(sys.argv)):
            ...
            if os.path.isdir(sys.argv[Argidx]): # 폴더 존재 유무 확인. 존재 하면 실행.
                ...
                CSVFile = open(sys.argv[Argidx] + "_" + DataExten[1:] + ".csv", 'w')
